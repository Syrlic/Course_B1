...
public class JamesHelper {
  ...
  public List<MailMessage> waitForMail(String name, String password, long timeout) throws MessagingException {
    long now = System.currentTimeMillis();
    while (System.currentTimeMillis() < now+timeout){
      List<MailMessage> allMail = getAllMail(name, password);
      if(allMail.size() > 0){
        return allMail;
      }
      try {
